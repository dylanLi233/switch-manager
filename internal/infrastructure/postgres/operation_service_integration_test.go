...
type scriptedSessionFactory struct {
	mu      sync.Mutex
	scripts [][]fakecli.Step
	opens   int
}
func (f *scriptedSessionFactory) add(steps ...fakecli.Step) { f.mu.Lock(); defer f.mu.Unlock(); f.scripts = append(f.scripts, steps) }
func (f *scriptedSessionFactory) Open(context.Context, device.Device) (operationsvc.Session, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if len(f.scripts) == 0 { return nil, errors.New("no fake CLI script configured") }
	steps := f.scripts[0]
	f.scripts = f.scripts[1:]
	f.opens++
	session, err := fakecli.New(steps, fakecli.Options{})
	if err != nil { return nil, err }
	return &closableFakeSession{inner: session}, nil
}
...
func TestOperationServicePostgreSQLIntegration(t *testing.T) {
	...
	if dsn == "" { t.Skip("TEST_DATABASE_DSN is not set") }
	...
	if err := registry.Register(plugin); err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	dispatcher, err := scheduler.New(repos.Tasks, executor, scheduler.Config{Workers: 1, PollInterval: 5 * time.Millisecond})
	if err != nil { t.Fatal(err) }
	runCtx, stop := context.WithCancel(context.Background())
	done := make(chan error, 1)
	go func() { done <- dispatcher.Run(runCtx) }()
	defer func() { stop(); if err := <-done; err != nil { t.Fatalf("scheduler run: %v", err) } }()

	service, err := operationsvc.NewService(repos.Tasks, repos.Audits, planner, dispatcher, operationsvc.Config{SyncWaitTimeout: 3 * time.Second, WaitPollInterval: 5 * time.Millisecond})
	if err != nil { t.Fatal(err) }
	actor := auth.Actor{UserID: operationUserID, Username: "operation-user", Role: auth.RoleAdmin, SourceIP: "192.0.2.1"}

	factory.add(
		fakecli.Step{Expect: pluginapi.PlannedCommand{Sequence: 1, Text: `fake.echo.config "hello"`, Timeout: 2 * time.Second}, Output: "configured"},
		fakecli.Step{Expect: pluginapi.PlannedCommand{Sequence: 1, Text: "fake.config.save", Timeout: 2 * time.Second}, Output: "partial", Failure: errors.New("save rejected"), ErrorCode: string(apperror.CodeCommandRejected)},
	)
	...
	configAudit, err := repos.Audits.Get(ctx, configSubmission.AuditID)
	if err != nil || configAudit.Status != string(task.StatusPartialSuccess) || configAudit.ErrorCode != string(apperror.CodeConfigSaveFailed) { t.Fatalf("config audit=%+v err=%v", configAudit, err) }

	opensBeforeDryRun := factory.openCount()
	dryRunSubmission, err := service.Submit(ctx, operationsvc.SubmitRequest{RequestID: "req-operation-dry", Operation: operation.Request{Name: operation.Name(fakeplugin.OperationEchoConfig), Class: operation.ClassConfig, DeviceID: operationDeviceID, Parameters: map[string]any{"message": "dry"}, ExecutionMode: operation.ExecutionModeSync, DryRun: true, Actor: actor}})
	if err != nil { t.Fatal(err) }
	if !dryRunSubmission.Completed || dryRunSubmission.Task.Status != task.StatusSuccess || factory.openCount() != opensBeforeDryRun { t.Fatalf("dry run task=%+v opens=%d", dryRunSubmission.Task, factory.openCount()) }
	var dryResult map[string]any
	if err := json.Unmarshal(dryRunSubmission.Task.Result, &dryResult); err != nil || dryResult["dry_run"] != true { t.Fatalf("dry result=%s err=%v", dryRunSubmission.Task.Result, err) }
	...
	querySubmission, err := service.Submit(ctx, operationsvc.SubmitRequest{RequestID: "req-operation-query", Operation: operation.Request{Name: operation.Name(fakeplugin.OperationEchoQuery), Class: operation.ClassQuery, DeviceID: operationDeviceID, Parameters: map[string]any{"message": "hello"}, ExecutionMode: operation.ExecutionModeAsync, Actor: actor}})
	if err != nil { t.Fatal(err) }
	queryTask := waitOperationTaskStatus(t, repos.Tasks, querySubmission.Task.ID, task.StatusSuccess)
	if queryTask.PluginName != "fake-huawei" || queryTask.PluginVersion != "1.0.0" { t.Fatalf("query task=%+v", queryTask) }
}

func waitOperationTaskStatus(t *testing.T, repository *TaskRepository, id string, want task.Status) task.Persisted {
	t.Helper()
	deadline := time.Now().Add(3 * time.Second)
	for {
		value, err := repository.Get(context.Background(), id)
		if err != nil { t.Fatal(err) }
		if value.Status == want { return value }
		if time.Now().After(deadline) { t.Fatalf("task %s status=%s want=%s", id, value.Status, want) }
