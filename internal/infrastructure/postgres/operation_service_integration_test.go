...
func TestOperationServicePostgreSQLIntegration(t *testing.T){
	...
	queryTask:=waitOperationTaskStatus(t,repos.Tasks,querySubmission.Task.ID,task.StatusSuccess);if queryTask.PluginName!="fake-huawei"||queryTask.PluginVersion!="1.0.0"{t.Fatalf("query task=%+v",queryTask)}
}
