...
func TestPartialTranscriptMustEndInFailure(t *testing.T) {
	...
	plan := ExecutionPlan{
		...
		Commands: []PlannedCommand{{Sequence: 1, Text: "one", Timeout: time.Second}, {Sequence: 2, Text: "two", Timeout: time.Second}},
	}
	transcript := Transcript{
		StartedAt: testTime(), FinishedAt: testTime().Add(time.Second),
		Commands: []CommandRecord{{Sequence: 1, Command: "one", Succeeded: true}},
	}
